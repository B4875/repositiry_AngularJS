...
angular.
  module('racunDetail3').
  component('racunDetail3', {
    templateUrl: 'racun-detail3/racun-detail3.template.html',
   controller: ['Klavzule', 'Partner', 'Racun', 'Doku', 'Udata', '$scope', '$http', 'moment', '$routeParams', '$location',
      function RacundetailKlController(Klavzule, Partner, Racun, Doku, Udata, $scope, $http, moment, route, $location) {
...
$scope.disabledDokuZap = 
	...
	if ((this.l1 != undefined) && (this.l2 != undefined)){
        ...
        $scope.uporabnik.uname = data.user.uname;
                ...
                $scope.klavzule0 = Klavzule.query({leto1:"01", leto2:"99", mesec1:"01", mesec2:"12", space9:'space1080', naslov:'tp2', usr:data.user.uname});
//pport:$scope.applist[appn].port, pnaslov:'tp9'});
	$scope.klavzule = {"SEZNAMKL":[{}]};
//	$scope.steviloKlavzul = 0;
	console.log($scope.partnerji);
		...
		console.log("PR0:" + $scope.PR0[0].KL[0].MAXKL);
		var leto0 = "21";
		var leto = ((new Date).toJSON()).substring(2,4);
		console.log("leto:"+leto0);
//		$scope.doku0 = Doku.query({leto:leto});
//		$scope.PR0[0].DOKU0 = (Doku.query({leto:leto, port:$scope.applist[appn].port, naslov:'tp7'}));
		$scope.PR0[0].DOKU0 = (Doku.query({leto:leto, space9:$scope.applist[appn].space9, naslov:'tp5'}));
				this.udata.$promise.then( function (data) {
				if ( ((data.user.username) == "PUBLIC") || ((data.user.uname) == "PUB00") || ((data.user.role).substr(1,2) != "pt")){
					//$scope.disabledDokuZap = true;
					$scope.publicReadOnly = "  -  uporabnik z omejenim dostopom (READ ONLY) ";
...
$scope.postData3 = function(doku, uporabnik) {
...
dp.d.toJSON = function(){ return moment(this).format(); }
	...
	if ($scope.PR0[0].KL[0].MAXKL > 0){
		dataj1["KLAVZULE"] = [];
		($scope.PR0[0].KL[0].SEZNAMKL).map(function(f){
			dataj1.KLAVZULE.push({"K": (f.K), "V":{"ID":(f.V.ID), "OZNAKA":(f.V.OZNAKA)}});
		});
	}
	...
	if ((this.l1 != undefined) && (this.l2 != undefined)){
		$http({		method: 'PUT',
		            url: 'https://server4875.si/space1280' + '/' + $scope.applist[appn].rnaslov + "?usr=" + uporabnik.uname,
		            data: {dataj1},
		...
		$http({		method: 'POST',
		            url: 'https://server4875.si/space1180' +  '/' + $scope.applist[appn].rnaslov + "?usr=" + uporabnik.uname,
		            data: {dataj1},
...
$scope.dodaj_klavzulo = function (klavzula) {

		if (($scope.PR0[0].KL[0].MAXKL) == 0)
			$scope.PR0[0].KL[0].SEZNAMKL.splice(0,1);
		($scope.PR0[0].KL[0].MAXKL)++;
		console.log("MAXKL:" + $scope.PR0[0].KL[0].MAXKL);
		var json1 = {"K":($scope.PR0[0].KL[0].MAXKL)};
//		var json2 = klavzula;
		json1["V"] = klavzula;
		$scope.PR0[0].KL[0].SEZNAMKL.push(json1);
            };

$scope.odstrani_klavzulo = function (klavzulanum) {
	$scope.PR0[0].KL[0].SEZNAMKL = ($scope.PR0[0].KL[0].SEZNAMKL).filter(function(f) { return f.K != klavzulanum });
};
