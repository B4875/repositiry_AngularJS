...
angular.
  module('tekstList').
  component('tekstList', {
    templateUrl: 'tekst-list/tekst-list.template.html',
   controller: ['Tekst', 'Udata', '$scope', '$routeParams',  
      function TekstListController(Tekst, Udata, $scope, route,$location) {
...
$scope.style4={"background-color":$scope.applist[appn].bcolor4};
    ...
    this.udata.$promise.then( function (data) {
        $scope.tekstlist = Tekst.query({space9:$scope.applist[appn].space9, naslov:'tp27', usr:data.user.uname});
        $scope.uporabnik.uporabnik = data.user.username;
    });

	$scope.nov_tekst = function(dat) {
		var url1 =  "https://server4875.si/#!/tekst?app=" + appn.toString();
		window.open(url1, "_blank", "width=1150, height=850");
	};

	$scope.get_details = function(id) {
		var url1 =  "https://server4875.si/#!/tekst?app=" + appn.toString() + "&id=" + id;
		window.open(url1, "_blank", "width=1150, height=850");
	};
